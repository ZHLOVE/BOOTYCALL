...
- (void)viewDidLoad {
    ...
    //    初始化音效
    NSURL *url = [[NSBundle mainBundle] URLForAuxiliaryExecutable:@"SystemSound.wav"];
...
-(void)didReceiveMessage:(EMMessage *)message
{
    ...
    NSDictionary *dict = [[NSUserDefaults standardUserDefaults]objectForKey:@"msgRemind"];
    
    if ([dict[@"jarStatus"] isEqualToString:@"1"] &&[dict[@"soundStatus"] isEqualToString:@"1"] ) {
        //        手机震动
        AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
        //        手机声SystemSoundID soundId;
        AudioServicesPlayAlertSound(_systemSoundID);
    }else if ([dict[@"jarStatus"] isEqualToString:@"0"] &&[dict[@"soundStatus"] isEqualToString:@"1"] ){
        
        //        手机声SystemSoundID soundId;
        AudioServicesPlayAlertSound(_systemSoundID);
    
    }else if ([dict[@"jarStatus"] isEqualToString:@"1"] &&[dict[@"soundStatus"] isEqualToString:@"0"] ){
        //        手机震动
        AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
                     ...
                     progress:(id<IEMChatProgressDelegate>)progress
{
    ...
    SCUSTOMSINGLETON.chatCount = self.messsagesSource.count;
     __weak EaseMessageViewController *weakSelf = self;
    dispatch_async(_messageQueue, ^{
