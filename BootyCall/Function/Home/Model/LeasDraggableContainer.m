...
@interface LeasDraggableContainer ()
 ...
 *  记录处于表面的对象
 */
@property (nonatomic,assign) NSInteger   indexWithface;


@end

@implementation LeasDraggableContainer
...
- (void)defaultConfig {
    ...
    self.direction = LeasDraggableDirectionDefault;
    self.loadedIndex = 0;
    self.moving = NO;
}
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfIndexs)] && [self.dataSource respondsToSelector:@selector(draggableContainer:viewForIndex:)]) {
        ...
        if (self.loadedIndex < indexs) {
            ...
            for (long int i = self.currentCards.count; i <  (self.moving ? preloadViewCont + 1: preloadViewCont); i++) {
                ...
                LeasDraggableCardView *cardView = [self.dataSource draggableContainer:self viewForIndex:self.loadedIndex];
...
- (void)tapGestureHandle:(UITapGestureRecognizer *)tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:didSelectIndex:)]) {
        [self.delegate draggableContainer:self cardView:(LeasDraggableCardView *)tap.view didSelectIndex:tap.view.tag];
    }
}
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        LeasDraggableCardView *cardView = (LeasDraggableCardView *)gesture.view;
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            ...
            if (widthRatio > 0) {
                self.direction = LeasDraggableDirectionRight;
            } if (widthRatio < 0) {
                self.direction = LeasDraggableDirectionLeft;
...
- (void)finishedPanGesture:(UIView *)cardView direction:(LeasDraggableDirection)direction scale:(CGFloat)scale disappear:(BOOL)disappear {
    ...
    if (!disappear) {
        ...
        // 2.重新布局剩下的cardViews
        // --------------------------------
        NSInteger flag = direction == LeasDraggableDirectionLeft ? -1 : 2;
        [UIView animateWithDuration:0.5f
                              delay:0.0
                            options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             cardView.center = CGPointMake(LGWidth * flag, LGWidth * flag / scale + self.cardCenter.y);
...
//                self.leftView = cardView;
//            }
//        }else if (flag == 2){
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"likeImageView" object:nil userInfo:@{@"like":cardView}];
//        }
        //=============== 选择对象
        if (self.delegate && [self.delegate respondsToSelector:@selector(selectedCardView:draggableDirection:didSelectIndex:)]) {
            [self.delegate selectedCardView:nil draggableDirection:direction didSelectIndex:_indexWithface];
            _indexWithface +=1;
        }
    }
}
...
- (void)removeFormDirection:(LeasDraggableDirection)direction {
    ...
    if (self.moving) {
        ...
        switch (direction) {
            case LeasDraggableDirectionLeft:
                cardCenter = CGPointMake(-LGWidth / 2, self.cardCenter.y);
                flag = -1;
                break;
            case LeasDraggableDirectionRight:
                cardCenter = CGPointMake(LGWidth * 1.5, self.cardCenter.y);
                flag = 1;
                break;
            default:
                break;
...
//            }
//        }else if (flag == 1){
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"likeImageView" object:nil userInfo:@{@"like":firstView}];
        //        }//=============== 选择对象
        //=============== 选择对象
        if (self.delegate && [self.delegate respondsToSelector:@selector(selectedCardView:draggableDirection:didSelectIndex:)]) {
            [self.delegate selectedCardView:nil draggableDirection:direction didSelectIndex:_indexWithface];
...
- (void)resetVisibleCards {
    ...
    __weak LeasDraggableContainer *weakself = self;
...
- (void)movingVisibleCards:(CGFloat)scale {
    ...
    for (int i = 1; i < self.currentCards.count; i++) {
        ...
        LeasDraggableCardView *cardView = [self.currentCards objectAtIndex:i];
