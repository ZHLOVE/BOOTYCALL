...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self initializeInterface:frame];
    }
    return self;
}

//初始化界面
- (void)initializeInterface:(CGRect)frame{
...
-(void)loadingAnimation{
    ...
    //[self performSelector:@selector(removeAllAnimation) withObject:self afterDelay:6];
...
-(void)removeAllAnimation{
    ...
    if (self.isSignInSuccess == NO) {
        
        [self initializeInterface:self.frame];
