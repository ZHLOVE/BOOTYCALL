...
//    self.navigationItem.title = @"SChatViewController";
...
- (void)_setupBarButtonItem
{
    ...
    backButton.imageEdgeInsets = UIEdgeInsetsMake(9, -10, 7, 25);
    [backButton setImage:[UIImage imageNamed:@"sback"] forState:UIControlStateNormal];
   ...
   didSelectAvatarMessageModel:(id<IMessageModel>)messageModel
{
    ...
    if (messageModel.isSender) {
        ...
        LeasUserDetailViewController  *detailVc = [[LeasUserDetailViewController alloc] init];
        
        detailVc.userID = messageModel.message.from;
        //NSLog(@"->>>>>>> %@",messageModel.message.from);
        [[SFoldawayButton sharSfoldawayButton] removeFromSuperview];
        detailVc.numberValues =SCUSTOMSINGLETON.chatCount;
...
//    NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:message.from];
    __block  NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:message.from];
    __block   NSString *avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:message.from];
//    NSLog(@"name%@",message.from);
    
    if (nickName && avatarURLPath) {
         model.nickname = nickName;
        model.avatarURLPath = avatarURLPath;
        
    } else {
        
        [MResponseUserInfo responseUserInfoWithId:message.from ResponseUserCompletion:^(BOOL success, NSError *error, NSDictionary *currentUserInfo) {
            [SCUserProfileEntity saveUserProfileWithUsername:message.from forNickName:currentUserInfo[@"myInfo"][@"nickName"] avatarURLPath:nil];
            //保存到本地后，从本地获取
            ...
            if (avatarURLPath) {
                ...
                [SCUserProfileEntity saveUserProfileWithUsername:message.from forNickName:nickName avatarURLPath:currentUserInfo[@"headImageUrl"]];
...
- (NSArray*)emotionFormessageViewController:(EaseMessageViewController *)viewController
{
    ...
    for (NSString *name in names) {
        index++;
        EaseEmotion *emotion = [[EaseEmotion alloc] initWithName:[NSString stringWithFormat:@"[兔斯基%d]",index] emotionId:[NSString stringWithFormat:@"em%d",(1000 + index)] emotionThumbnail:[NSString stringWithFormat:@"%@_cover",name] emotionOriginal:[NSString stringWithFormat:@"%@",name] emotionOriginalURL:@"" emotionType:EMEmotionGif];
